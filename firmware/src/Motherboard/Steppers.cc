...
void reset() {
	...
	if ( acceleration ) {
		...
		axis_steps_per_unit[X_AXIS] = convertAxisMMToFloat(eeprom::getEepromStepsPerMM(mbeeprom::STEPS_PER_MM_X, STEPS_PER_MM_X_DEFAULT));
		axis_steps_per_unit[Y_AXIS] = convertAxisMMToFloat(eeprom::getEepromStepsPerMM(mbeeprom::STEPS_PER_MM_Y, STEPS_PER_MM_Y_DEFAULT));
		axis_steps_per_unit[Z_AXIS] = convertAxisMMToFloat(eeprom::getEepromStepsPerMM(mbeeprom::STEPS_PER_MM_Z, STEPS_PER_MM_Z_DEFAULT));
		axis_steps_per_unit[E_AXIS] = (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_E_STEPS_PER_MM,44) / 10.0;
 
		//M201 - Set max acceleration in units/s^2 for print moves
		// X, Y, Z, E maximum start speed for accelerated moves. E default values are good for skeinforge 40+, for older versions raise them a lot.
		max_acceleration_units_per_sq_second[X_AXIS] = eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_ACCELERATION_X, 2000);
		max_acceleration_units_per_sq_second[Y_AXIS] = eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_ACCELERATION_Y, 2000);
		max_acceleration_units_per_sq_second[Z_AXIS] = eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_ACCELERATION_Z, 150);
		max_acceleration_units_per_sq_second[E_AXIS] = eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_ACCELERATION_A, 60000);

		for (uint8_t i = 0; i < NUM_AXIS; i ++)
			axis_steps_per_sqr_second[i] = max_acceleration_units_per_sq_second[i] * axis_steps_per_unit[i];
		
		//M203 - Set maximum feedrate that your machine can sustain in mm/sec
		max_feedrate[X_AXIS] = (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_FEEDRATE_X, 160);
		max_feedrate[Y_AXIS] = (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_FEEDRATE_Y, 160);
		max_feedrate[Z_AXIS] = (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_FEEDRATE_Z, 10);
		max_feedrate[E_AXIS] = (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_FEEDRATE_A, 100);
		...
		p_acceleration		= (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_EXTRUDER_NORM, 5000);

		// X, Y, Z and E max acceleration in mm/s^2 for r retracts
		p_retract_acceleration	= (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_EXTRUDER_RETRACT, 3000);

		//M205 - Advanced Settings
		//minimumfeedrate   - minimum travel speed while printing
		minimumfeedrate	  = (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_MIN_FEED_RATE,0)	 / 10.0;

		//mintravelfeedrate - minimum travel speed while travelling
    		mintravelfeedrate = (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_MIN_TRAVEL_FEED_RATE,0) / 10.0;

		//max_xy_jerk     - maximum xy jerk (mm/sec)
    		max_xy_jerk	  = (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_XY_JERK,2)	 / 10.0;
		max_xy_jerk_squared = max_xy_jerk * max_xy_jerk;

		//max_z_jerk        - maximum z jerk (mm/sec)
    		max_z_jerk	  = (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_MAX_Z_JERK,100)	 / 10.0;

    		float advanceK	 	= (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_ADVANCE_K,50)		/ 100000.0;
    		float filamentDiameter  = (float)eeprom::getEepromUInt32(mbeeprom::ACCEL_FILAMENT_DIAMETER,175)	/ 100.0;
