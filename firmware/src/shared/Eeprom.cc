...
void init() {
        uint8_t version[2];
        eeprom_read_block(version,(const uint8_t*)sharedeeprom::VERSION_LOW,2);
        if ((version[1]*100+version[0]) == firmware_version) return;
        if (version[1] == 0xff || version[1] < 2) {
#ifdef MOTHERBOARD
            mbeeprom::setDefaults();
#endif
#ifdef EXTRUDER         
            extrudereeprom::setDefaults();
#endif
        }
        // Write version
        version[0] = firmware_version % 100;
        version[1] = firmware_version / 100;
        eeprom_write_block(version,(uint8_t*)sharedeeprom::VERSION_LOW,2);
}
